...
def train():
    ...
    # bert running forever，这里为了加快check debug，改成前1000行
    dfx = pd.read_csv(config.TRAINING_FILE).fillna("none")[:1000]
    ...
    df_train, df_valid = model_selection.train_test_split(
        dfx,
        test_size=0.1, random_state=42, stratify=dfx.sentiment.values)
    ...
    for epoch in range(config.EPOCHS):
        engine.train_fn(
            train_data_loader, model, optimizer, device, scheduler, writer, epoch
        )
